...
(function ($) {
	...
	var QRCode = function (text, level, version, quiet) {
		...
		drawModules = function (qr, context, settings) {
			...
			var moduleSize = (settings.moduleSize > 0) ? settings.moduleSize : Math.floor(settings.size / moduleCount);
			var fn = drawModuleDefault;
			var row;
			var col;

			settings.left = Math.round((settings.size - (moduleSize * moduleCount)) / 2);
			settings.top = Math.round((settings.size - (moduleSize * moduleCount)) / 2);
